...
pub async fn get_document<T: Document>(
    ...
    match res {
        Ok(value) => {
            let index: Result<T, serde_json::error::Error> =
                serde_json::from_value(value) as Result<T, serde_json::error::Error>;
            match index {
                Ok(data) => Ok(data),
...
pub async fn add_or_replace<T: Document>(
    ...
    match res {
        Ok(value) => {
            let index: Result<DocumentState, serde_json::error::Error> =
                serde_json::from_value(value) as Result<DocumentState, serde_json::error::Error>;
            match index {
                Ok(data) => Ok(data),
                Err(err) => Err(ServiceError::new(
                    StatusCode::INTERNAL_SERVER_ERROR,
